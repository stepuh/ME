...
public abstract class AbstractEM {
	ArrayList<Pattern> patterns;	// set of train data
	...
	public ArrayList<Pattern> generatePrototypes(int n){
		ArrayList<Pattern> prototypes = new ArrayList<Pattern>();
		for(int i=0; i<n; i++){
			double[] features = new double[dimensions];
			for(int j=0; j<dimensions; j++){
				features[j] = Math.random() *100;
			}
			prototypes.add( new Pattern(features) );
		}
		return prototypes;
	}
